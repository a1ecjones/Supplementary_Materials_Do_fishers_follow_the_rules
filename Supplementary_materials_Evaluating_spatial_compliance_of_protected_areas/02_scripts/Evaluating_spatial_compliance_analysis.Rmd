---
title: Evaluating spatial compliance of protected areas by recreational fishers and resulting ecological responses of temperate rocky reefs
author: "Alec Jones"
date: "2025-12-03"
...
This document contains the code necessary to run the analyses performed in "Evaluating spatial compliance of protected areas by recreational fishers and resulting ecological responses of temperate rocky reefs"
...
(total_rec_boats_map_2023_test <- ggplot()+
  ...
  geom_point(data = rec_boats_only_fishing2023_subset_clean, 
...
```{r total rec boats within 1.5 km 2021}
(total_rec_boats_map_2021_test <- ggplot()+
    geom_sf(data = VI_base_layer_hi_res_sf_subset, colour = "darkgrey", fill = "lightgrey")+
  geom_point(data = rec_boats_only_fishing2021_subset_clean, 
             mapping = aes(x = Longitude, y = Latitude),
             size = 0.8, fill = "#070707")+
      geom_sf(data = broken_group_RCA, fill = "blue", colour ="blue", alpha = 0.1)+
   geom_sf(data = numukamis_area, fill = "blue", colour = "blue", alpha = 0.2)+
...
```{r total rec boats within 1.5 km 2022}
(total_rec_boats_map_2022_test <- ggplot()+
    geom_sf(data = VI_base_layer_hi_res_sf_subset, colour = "darkgrey", fill = "lightgrey")+
  geom_point(data = rec_boats_only_fishing2022_subset_clean, 
             mapping = aes(x = Longitude, y = Latitude),
             size = 0.8, fill = "#070707")+
      geom_sf(data = broken_group_RCA, fill = "blue", colour ="blue", alpha = 0.1)+
   geom_sf(data = numukamis_area, fill = "blue", colour = "blue", alpha = 0.2)+
...
#Converting the other aspects of the map into the same projection as the KDE
...
rec_boat_kde_overall_sparr <- bivariate.density(rec_boats_ppp,
                                                ...
                                                #Specifies the type of edge correction to use. Diggle reweights each observation-specific kernel
                                                edge = "diggle",
                                                #True makes the function use the adaptive bandwith method
                                                adapt = TRUE,
                                                #True makes a progress bar show below
                                                verbose = TRUE,
                                                #Specifies the resolution of the evaluation grid that the density/intensity will be returned on
                                                resolution = 250,
                                                #True makes the function return an intensity estimate instead of density
                                                intensity = TRUE)
...
In order to plot the KDEs I needed to convert them back to objects usable with the ggplot package.
...
#Converting the intensity to units of per km squared from per m squared
...
#These lines identify the datapoints in the top 20 and 5% of values so contour lines can be plotted on the figure
...
The plot is showing the intensity of boats. Intensity is the average number of points (rec boats) per unit area. The density is proportionate to the intensity but is scaled so that all values sum to 1. The density surface is a probability density function of the rec boat data.
...
in the plots above, we wanted to use stats to actually see if there was a significant difference
between protected and unprotected sites. Due to the non-normal distributions of the boat counts, we used a Wilcoxon rank-sum test.
...
all of our groups we need a non-parametric test. Instead we used 
...
I will be using the glmmTMB and sdmTMB packages to create models looking at the abundances/densities of urchins, fish, and other invertebrates at the RLS sites within Barkley Sound. The models will be based on the protection status of the survey sites as the predictor variable.

For simplicity I will only include the final model used for each species below.

## Set up for doing spatial analyses on the data

To be able to use sdmTMB I needed to make a spatial mesh which was done in the following chunks.
...
I tried a few different distances for the cutoff but 1 km gave the best balance between resolution and overfitting.
...
The model used was fit with sdmTMB. 
...
```{r Testing Fish ab spatiotemp model fit}
set.seed(123)
...
#Since there are multiple transcets from the same sites their residuals need to be grouped and averaged. This groups them
group_locations <- aggregate(overall_rls_data_final[, c("easting_km", "northing_km")], list(site_group), mean)
#This recalculates the residuals with the grouping structure
fish_ab_spatiotemp_resids_2 <- recalculateResiduals(r_fish_ab_spatiotemp, group = site_group)
#This tests for spatial autocorrelation
...
In this model I tested the transect level species richness of fish to see if there were differences between protected and unprotected transects. I started by making a dataframe that only had fish, and then counted the unique species observed at each transect.

#### Making a DF of unique fish species
...
For fish species richness I was able to use glmmTMB as there was not the same issue of autocorrelation of residuals.
```{r Fish sp rich glmm site}
fish_sp_rich_glmm_site <- glmmTMB(fish_sp_rich ~ protection_status + (1|site_code),#Adding site as a random intercept
                             data = fish_sp_rich_df,
                             family = glmmTMB::nbinom2(link = "log"))

fish_sp_rich_glmm_site

#Using DHARMa to look at model fit
...
```{r Testing for spatial autocorrelation for fish sp rich mod site}
#For this I made another df of coordinates from the fish sp richness df just to be safe
fish_sp_rich_df_coords <- st_as_sf(fish_sp_rich_df, coords = c("longitude", "latitude"), crs = 4326)
fish_sp_rich_df_coords <- st_transform(fish_sp_rich_df_coords, 32610)

fish_sp_rich_df_coords_extracted <- st_coordinates(fish_sp_rich_df_coords)
fish_sp_rich_df_coords$easting <- fish_sp_rich_df_coords_extracted[,1]
fish_sp_rich_df_coords$northing <- fish_sp_rich_df_coords_extracted[,2]
fish_sp_rich_df_coords$easting_km <- fish_sp_rich_df_coords$easting / 1000
fish_sp_rich_df_coords$northing_km <- fish_sp_rich_df_coords$northing / 1000

fish_sp_rich_df <- fish_sp_rich_df%>%
 left_join(fish_sp_rich_df_coords, by = c("site_code", "site_name", "protection_status",
                                 "survey_date", "depth", "fish_sp_rich"))
fish_sp_rich_glmm_site_resids <- simulateResiduals(fish_sp_rich_glmm_site)
...
To look at B20 impacts we needed to make a data frame with only observations of fish greater than 20 cm in length, and then have counts or biomass for each transect.
...
b20_df <- overall_rls_data_raw_lat_long_updated_names%>%
  ...
  #These filter to only have fish in the df
  filter(phylum == "Chordata")%>%
  filter(class != "Mammalia")%>%
...
b20_final_df <- b20_df%>%
  ...
  #dividing by 1000 to make biomass in kg
...
```{r}
hist(b20_final_df$b20_biomass[b20_final_df$protection_status == "protected"])
hist(b20_final_df$b20_biomass[b20_final_df$protection_status == "unprotected"])
```
...
```{r Testing the B20 model for spatial autocorrelation}
#For this I made another df of coordinates from the b20 df just to be safe
...
```{r Testing for spatial autocorrelation}
urch_mod_glmm_site_resids <- simulateResiduals(urch_mod_glmm_site)
...
```{r Testing for spatial autocorrelation for dec mod RE}
dec_mod_glmm_site_resids <- simulateResiduals(dec_mod_glmm_site)
...
Effingham west has very large outliers (outside 1.5x interquartile range) and is causing issues with model fitting. I ran the model with and without Effingham west and the trends were very similar.
...
This is the model with Effingham West removed.
```{r Snail mod glmm site no outliers}
...
```{r Testing for spatial autocorrelation for snail mod no outliers}
site_group <- overall_rls_data_final$site_code
group_locations <- aggregate(overall_rls_data_final[, c("easting_km", "northing_km")], list(site_group), mean)
...
### Snail glmm site with outliers

To ensure the results were consistent with and without the outliers here is the model with Effingham West included.
...
```{r Testing for spatial autocorrelation for snail mod no RE}
site_group <- overall_rls_data_final$site_code
group_locations <- aggregate(overall_rls_data_final[, c("easting_km", "northing_km")], list(site_group), mean)
...
## Sunflower sea star (Pycnopodia helianthoides) modelling
...
```{r Testing for spatial autocorrelation for pycno mod}
site_group <- overall_rls_data_final$site_code
group_locations <- aggregate(overall_rls_data_final[, c("easting_km", "northing_km")], list(site_group), mean)
...
To make plots for each of the models I first predicted marginal means and 95% confidence intervals for each model and then plotted those on boxplots between protection status.
...
#Using emmeans to get estimated marginal means for fish abundance between protected and unprotected sites on the response scale
#Getting the estimated mean fish abundance for each level of protection status
fish_ab_sum_stats <- emmeans::emmeans(fish_ab_spatiotemp, ~ protection_status, type = "response")

#Confint gives confidence intervals for the estimates
...
#Using ggpredict to get model predictions for species richness at protected and unprotected sites
...
## Combining the plots into a multipanel plot
...
I further refined the plot in inkscape and added some icons for the different species.
