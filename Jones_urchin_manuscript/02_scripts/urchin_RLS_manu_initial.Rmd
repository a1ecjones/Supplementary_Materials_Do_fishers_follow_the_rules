...
```{r}
fish_test_stat <- abs(mean(overall_rls_data_expanded$size_class[overall_rls_data_expanded$protection_status == "protected"]) -
                        mean(overall_rls_data_expanded$size_class[overall_rls_data_expanded$protection_status == "unprotected"]))


set.seed(15)

variable <- overall_rls_data_expanded$size_class

protected_index <- which(overall_rls_data_expanded$protection_status == "protected")

unprotected_index <- which(overall_rls_data_expanded$protection_status == "unprotected")
...
for (i in seq_len(P)) {
  ...
  fish_perm_test_stat[i] <- abs(mean(variable[perm][protected_index]) - mean(variable[perm][unprotected_index]))
}
