...
## Adding columns for binning the fishing boat impacts

```{r Adding columns for binning the fishing boat impacts}
...
I will try with a few different likely distributional families and then see which ones fits the data the best.

### Tweedie

```{r Urchin model}
urch_mod <- sdmTMB(total_urchins ~ protection_status + depth + total_fish,
...
In order to visualize how impacted each RLS site in Barkley Sound is by recreational fishing boats I am making a map of the sound with the impacts of each site represented by colour and size of point. The colours, and sizes will be binned into three or four categories so for ease of viewing.
...
#This is used to adjust the size of the points to be scaled based on the values
    ...
    limits = c(0, 120),
    #This code adds the labels from the size breaks object and adds a plus sign
    #to the biggest label indicating there can be larger values
    labels = c(scales::comma(size_breaks_1km[-length(size_breaks_1km)]),
               paste0(scales::comma(size_breaks_1km[length(size_breaks_1km)]), "+")),
      name = "Mean Rec Boat\nCounts (1 km)")+
...
(rec_boat_impacts_1.5km_mean <- ggplot()+
   ...
   #this plots the RLS sites
    ...
    limits = c(0, 120),
    labels = c(scales::comma(size_breaks_1.5km[-length(size_breaks_1.5km)]),
               paste0(scales::comma(size_breaks_1.5km[length(size_breaks_1.5km)]), "+")),
      name = "Mean Rec Boat\nCounts (1.5 km)")+
 scale_fill_manual(values = c("#F46864", "#1CB8BC"))+
    coord_sf(xlim = c(long_min + 0.02, long_max - 0.035),
           ylim = c(lat_min + 0.01, lat_max - 0.03)) +
