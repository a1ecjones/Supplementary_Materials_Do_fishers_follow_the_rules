...
max_fish_size_df <- overall_rls_data_expanded%>%
  ...
  filter(size_class == max(size_class, na.rm = TRUE))%>%
  ungroup()%>%
...
```{r Max fish size spatiotemp}
max_fish_size <- glmmTMB(
  data = max_fish_size_df,
  formula = size_class ~ protection_status + (1|reporting_name) + (1|site_code),
  family = glmmTMB::tweedie(link = "log")
)

max_fish_size_resids <- simulateResiduals(max_fish_size)
plot(max_fish_size_resids)
```
