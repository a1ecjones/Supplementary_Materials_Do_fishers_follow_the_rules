...
(overall_rec_boat_kde_sparr_map <- ggplot()+
  ...
  geom_sf(data = VI_base_layer_hi_res_sf_subset, colour = "darkgrey", fill = "lightgrey")+
      geom_sf(data = broken_group_RCA, fill = "white", colour ="white", alpha = 0.1)+
   geom_sf(data = numukamis_area, fill = "white", colour = "white", alpha = 0.2)+
   geom_sf(data = baeria_rocks_er, fill = "white", colour = "white", alpha = 0.2)+
...
mean_proc_urch <- mean(overall_rls_data_final$Mesocentrotus_franciscanus[overall_rls_data_final$protection_status == "protected"])
sd_proc_urch <- sd(overall_rls_data_final$Mesocentrotus_franciscanus[overall_rls_data_final$protection_status == "protected"])
se_proc_urch <- sd_proc_urch/sqrt(length(overall_rls_data_final$Mesocentrotus_franciscanus[overall_rls_data_final$protection_status == "protected"]))

mean_unproc_urch <- mean(overall_rls_data_final$Mesocentrotus_franciscanus[overall_rls_data_final$protection_status == "unprotected"])
sd_unproc_urch <- sd(overall_rls_data_final$Mesocentrotus_franciscanus[overall_rls_data_final$protection_status == "unprotected"])
se_unproc_urch <- sd_unproc_urch/sqrt(length(overall_rls_data_final$Mesocentrotus_franciscanus[overall_rls_data_final$protection_status == "unprotected"]))

mean_total_urch <- mean(overall_rls_data_final$Mesocentrotus_franciscanus)
sd_total_urch <- sd(overall_rls_data_final$Mesocentrotus_franciscanus)
se_total_urch <- sd_total_urch/sqrt(length(overall_rls_data_final$Mesocentrotus_franciscanus))
```
...
urch_mod_glmm_site <- glmmTMB(Mesocentrotus_franciscanus ~ protection_status + (1|site_code),
                         data = overall_rls_data_final,
                         family = gaussian(link = "identity"))
...
snail_mod_glmm_site <- glmmTMB(Pomaulax_gibberosus ~ protection_status + (1 + protection_status|site_code),
                         data = overall_rls_data_final,
                         family = glmmTMB::nbinom2(link = "log"))
...
snail_ab_sum_stats <- ggpredict(snail_mod_glmm_site,
                               terms = c("protection_status", "site_code"),
                               type = "random",
                               bias_correction = TRUE) %>%
  dplyr::rename(protection_status = x,
