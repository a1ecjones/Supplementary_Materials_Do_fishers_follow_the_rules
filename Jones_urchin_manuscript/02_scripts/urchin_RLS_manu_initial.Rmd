...
```{r Pivoting fish data to wide form}
overall_rls_data_sizes_raw <- overall_rls_data_raw_lat_long%>%
   select(-c("FID", "survey_id", "country", "area", "ecoregion", "realm", "location", "program",
            "survey_latitude", "survey_longitude"))%>%
  filter(phylum == "Chordata")%>%
   group_by(site_code, site_name, latitude, longitude, survey_date, depth, size_class)%>%
  summarise(site_count_by_size = sum(total))

overall_rls_data_sizes_raw$size_class <- as.factor(overall_rls_data_sizes_raw$size_class)

overall_rls_data_sizes_raw_wide <- overall_rls_data_sizes_raw%>%
  pivot_wider(
    names_from = size_class,
    values_from = site_count_by_size,
    values_fill = 0
  )
```

```{r Cutting the unnecessary sites}
#Cutting the other sites

overall_rls_data_clean_sizes <- overall_rls_data_sizes_raw_wide%>%
  filter(site_code %in% c("BMSC1", "BMSC10", "BMSC11", "BMSC12", "BMSC13", "BMSC14", "BMSC15",
                          "BMSC16", "BMSC17", "BMSC18", "BMSC19", "BMSC2", "BMSC20", "BMSC21",
                          "BMSC22", "BMSC23", "BMSC24", "BMSC25", "BMSC26", "BMSC27", "BMSC3", "BMSC4", "BMSC5", "BMSC6", "BMSC7",
                          "BMSC8", "BMSC9"))%>%
...
overall_rls_data_clean_sizes$site_code <- as.factor(overall_rls_data_clean_sizes$site_code)
overall_rls_data_clean_sizes$site_name <- as.factor(overall_rls_data_clean_sizes$site_name)
overall_rls_data_clean_sizes$protection_status <- as.factor(overall_rls_data_clean_sizes$protection_status)
```

```{r Adding a column for year to the sizes df}
#Extracting the year from the date column and then making a new column called
#year
overall_rls_data_clean_sizes$year <- format(overall_rls_data_clean_sizes$survey_date, "%Y")

#reordering the df
overall_rls_data_clean_sizes <- overall_rls_data_clean_sizes[,c(seq(1, 5, by = 1), 27,
                                                    seq(6, 26, by = 1))]

overall_rls_data_clean_sizes$year <- as.numeric(overall_rls_data_clean_sizes$year)
```

```{r Adding a column for site kelp characteristics to the size df}
overall_rls_data_clean_sizes <- overall_rls_data_clean_sizes%>%
  mutate(kelp = case_when(
    site_code %in% c("BMSC15", "BMSC26") ~ "Forest",
    site_code %in% c("BMSC10") ~ "Prior forest",
    site_code %in% c("BMSC22", "BMSC4", "BMSC17") ~ "Prior understory",
...
#overall_rls_data_final <- overall_rls_data_final[, c(1, 2, 191, seq(3, 190, by = 1))]
```

```{r Combining the groups into either presence or absence of macroalgae for the size df}
overall_rls_data_clean_sizes <- overall_rls_data_clean_sizes%>%
  mutate(kelp_pa = case_when(
    kelp %in% c("Forest", "Understory") ~ "Present",
    kelp %in% c("Prior forest", "Prior understory", "Barren") ~ "Absent"
...
#overall_rls_data_final <- overall_rls_data_final[, c(1, 2, 3, 192, seq(4, 191, by = 1))]
```
...
```{r Combining the groups into either presence or absence of macroalgae}
overall_rls_data_final <- overall_rls_data_final%>%
  mutate(kelp_pa = case_when(
    kelp %in% c("Forest", "Understory") ~ "Present",
    kelp %in% c("Prior forest", "Prior understory", "Barren") ~ "Absent"
...
ggplot(overall_rls_data_final, aes(x = kelp_pa
                                   
                                   , y = Rhinogobiops_nicholsii))+
  geom_boxplot()+
  theme.figure()
```
