...
Residuals also look quite bad for the spatial model. Possibly poisson is not correct.
...
I will be starting with a model looking at the red sea urchins only, as preliminary analyses show that the numbers of other urchin species are neglible compared to the red urchins.
...
#urch_mod_no_spatial_resids <- simulateResiduals(urch_mod_no_spatial)
#plot(urch_mod_no_spatial_resids)
```
...
urch_mod_glmm <- glmmTMB(Mesocentrotus_franciscanus ~ protection_status + (1|site_code),
                         data = overall_rls_data_final,
...
urch_mod_glmm_resids <- residuals(urch_mod_glmm)

#creating the spatial weights matrix
coords_for_auto_test <- st_as_sf(overall_rls_data_final, coords = c("longitude", "latitude"), crs = 4326)
...
nb <- spdep::dnearneigh(extracted_coords_for_auto_test_projected, d1 = 0, d2 = 10)
...
spdep::moran.test(urch_mod_glmm_resids, lw,
...
lw_inverse <- spdep::mat2listw(inverse_dist_matrix, style = "W")
spdep::moran.test(urch_mod_glmm_resids, lw_inverse,
           alternative = "two.sided")
```
