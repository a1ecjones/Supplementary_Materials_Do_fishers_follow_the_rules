...
#### Fish ab spatial glmm

```{r Fish ab spatial glmm model}
fish_ab_spatial_glmm <- glmmTMB(
  data = overall_rls_data_final,
  formula = total_fish ~ protection_status + (1|site_code) + (1|year) +
    ,
  family = glmmTMB::nbinom2(link = "log")
)

fish_ab_spatial_glmm
summary(fish_ab_spatial_glmm)


fish_ab_spatial_glmm_resids <- residuals(fish_ab_spatial_glmm)
qqnorm(fish_ab_spatial_glmm_resids)
```



```{r Testing if including protection status improves the model}
AIC(fish_ab_spatiotemp,
    fish_ab_spatiotemp_null)%>%
  arrange(AIC)%>%
